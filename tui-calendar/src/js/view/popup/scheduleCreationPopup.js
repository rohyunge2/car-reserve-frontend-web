...
function ScheduleCreationPopup(container, calendars, usageStatistics) {
    ...
    domevent.on(container, 'keypress', this._onKeyPress, this);
}
 ...
 * @override
 * 엔터키로 저장 및 ESC로 닫기
 * @param {KeyEvent} keyupEvent - key event object
 */
 ScheduleCreationPopup.prototype._onKeyPress = function(keyupEvent) {
    if ( keyupEvent.code == "Enter" ){
        this._onClickSaveSchedule(document.querySelector(".tui-full-calendar-popup-save"));
    } else if ( keyupEvent.code == "Escape" ){
        this._closePopup(document.querySelector(".tui-full-calendar-ic-close"));
    }
};
...
ScheduleCreationPopup.prototype._onClickSaveSchedule = function(target) {
    ...
    purpose = domutil.get(cssPrefix + 'schedule-title');

    startDate = new TZDate(this.rangePicker.getStartDate());
    endDate = new TZDate(this.rangePicker.getEndDate());

    var userName = domutil.get(cssPrefix + 'user_name');
    var destinationArea = domutil.get(cssPrefix + 'destination-area');
    var carNumber = domutil.get(cssPrefix + 'carNumber').innerText;
    
    // if (!this._validateForm(title, startDate, endDate, etc)) {
        // if (!title.value) {
        //     title.focus();
        // }
        // return false;
    ...
    if (!startDate && !endDate) {
        ...
        return false;
    } else if ( (this._selectedCal ? this._selectedCal.id : null) == "None" ){
        alert("부서선택은 필수항목 입니다.");
        return false;
    } else if ( carNumber == "차량선택" || carNumber == "" ){
        alert("차량선택은 필수항목 입니다.");
        return false;
    } else if ( !userName.value ) {
        alert("예약자명은 필수항목 입니다.");
        return false;
    } else if ( !destinationArea.value) {
        alert("행선지역은 필수항목 입니다.");
        return false;
    } else if ( !purpose.value ) {
        alert("운행목적은 필수항목 입니다.");
        return false;
    }
...
ScheduleCreationPopup.prototype.render = function(viewModel) {
    ...
    if ( !viewModel.carNumber ) {
        viewModel.carNumber = "차량선택";
    }
    ...
    let cloneNode = document.querySelector("#carInfoDataOri").cloneNode(true);
    let creationCarInfoNode = document.querySelector("#carInfoData");
    let divDepCarInfo = document.querySelector("#divDepCarInfo");

    cloneNode.id = "carInfoData";
    cloneNode.querySelector("#tui-full-calendar-carNumberOri").innerHTML = viewModel.carNumber;
    cloneNode.querySelector("#tui-full-calendar-carNumberOri").id = "tui-full-calendar-carNumber";
    cloneNode.querySelector("#carNumbersOri").id = "carNumbers";

    creationCarInfoNode.remove();
    divDepCarInfo.appendChild(cloneNode);
...
ScheduleCreationPopup.prototype._makeEditModeData = function(viewModel) {
    ...
    var title, userName, place, purpose, destinationArea, isPrivate, location, startDate, endDate, isAllDay, state, place_1_checked, place_2_checked, carNumber;
    ...
    place_1_checked = schedule.place == 1;
    place_2_checked = schedule.place == 2;
    ...
    return {
        ...
        purpose : purpose,
        destinationArea : destinationArea,
        place_1_checked: place_1_checked,
        place_2_checked: place_2_checked,
        place: place,
        isPrivate: isPrivate,
        location: location,
        isAllDay: isAllDay,
 ...
 */
ScheduleCreationPopup.prototype._validateForm = function(title, startDate, endDate, etc) {
    // if (!title.value) {
    //     return false;
    // }
    ...
    if ( Array.isArray(etc) ){
        return etc.every((element,index) => {
            if ( element ){
                return true;
            } else {
                return false;
...
ScheduleCreationPopup.prototype._onClickUpdateSchedule = function(form) {
    var changes = common.getScheduleChanges(
        this._schedule,
        ['calendarId', 'title', 'userName', 'purpose', 'destinationArea', 'location', 'start', 'end', 'isAllDay', 'state', 'carNumber'],
        {
