...
var helpers = {
    ...
    'alldayTitle-tmpl': function() {
        ...
        return '<span class="' + className + '">종일</span>';
    ...
    'purpose-tmpl': function(model) {
        return common.stripTags(model.purpose);
    },

    'carNumber-tmpl': function(model) {
        return common.stripTags(model.carNumber);
    },

    'userName-tmpl': function(model) {
        return common.stripTags(model.userName);
    },

    'destinationArea-tmpl': function(model) {
        return common.stripTags(model.destinationArea);
    },

    'meeting-time-tmpl': function(model) {
        return common.stripTags(model.getStarts().getHours() + ":" + (model.getStarts().getMinutes() < 10 ? "0"+model.getStarts().getMinutes() : model.getStarts().getMinutes()) + " ~ " + model.getEnds().getHours() + ":" +  (model.getEnds().getMinutes() < 10 ? "0"+model.getEnds().getMinutes() : model.getEnds().getMinutes()) );
    },
    ...
    'popupDetailDate-tmpl': function(isAllDay, start, end) {
        ...
        var endFormat = (isSameDate ? '' : 'MM월 DD일 ') + 'tt hh:mm';

        if (isAllDay) {
            return datetime.format(start, 'MM월 DD일') + (isSameDate ? '' : ' - ' + datetime.format(end, 'MM월 DD일'));
        }

        // return (datetime.format(start, 'YYYY.MM.DD hh:mm tt') + ' - ' + datetime.format(end, endFormat));
        var result = (datetime.format(start, 'MM월 DD일 tt hh:mm') + ' - ' + datetime.format(end, endFormat));
        result = result.replace(/am/gi, "오전");
        result = result.replace(/pm/gi, "오후");
        return result;
    },
