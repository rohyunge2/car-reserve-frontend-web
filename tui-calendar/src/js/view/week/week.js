...
Week.prototype.render = function() {
    ...
    if( viewModel.schedulesInDateRange.allday.length > 0 ) {
        for ( let allday of viewModel.schedulesInDateRange.allday ){
            let vModel = allday[0][0];
            if ( vModel.model.isReturn == "1" ) {
                vModel.model.bgColor = "#777";
                vModel.model.dragBgColor = "#777";
            }

            if ( new Date(vModel.model.end._date).getTime() - new Date().getTime() < 0 ){
                vModel.model.customStyle = "opacity: 0.4;"
            }
        }
    }
